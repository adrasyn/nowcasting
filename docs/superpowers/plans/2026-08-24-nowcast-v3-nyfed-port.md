...
def _ssm_from(data, prefix):
    return StateSpace(
        D=data[f"{prefix}__D"], H=data[f"{prefix}__H"],
        Sigma_eps=data[f"{prefix}__Sigma_eps"], C=data[f"{prefix}__C"],
        F=data[f"{prefix}__F"], G=data[f"{prefix}__G"],
        Sigma_eta=data[f"{prefix}__Sigma_eta"], mu_1=data[f"{prefix}__mu_1"],
        Sigma_1=data[f"{prefix}__Sigma_1"],
    )
...
def test_kalman_matches_octave_on_small_system(fixture):
    ...
    got = kalman_filter(d["Y"], _ssm_from(d, "SSM"), need_loglik=True)
    assert got.loglik == pytest.approx(float(d["ll"]), rel=1e-10)
    assert np.allclose(got.error, d["pred__error"], rtol=1e-10, equal_nan=True)
    assert np.allclose(got.gain, d["pred__gain"], rtol=1e-10, equal_nan=True)


@pytest.mark.fixtures
def test_kalman_matches_octave_on_the_us_panel(fixture):
    """73 states, time-varying H and Sigma_eta, ragged edge."""
    d = fixture("kalman_us")
    got = kalman_filter(d["Y"], _ssm_from(d, "SSM"))
    assert np.allclose(got.error, d["pred__error"], rtol=1e-10, equal_nan=True)
    assert np.allclose(got.inv_mse, d["pred__invMSE"], rtol=1e-10, equal_nan=True)


@pytest.mark.fixtures
def test_fast_smoother_matches_octave(fixture):
    d = fixture("fast_smoother_us")
    got = fast_smoother(d["Y"], _ssm_from(d, "SSM"))
...
def test_transition_matrices_are_indexed_one_period_back(fixture):
    """Guards the t vs t-1 off-by-one. Feeding a time-varying F whose slices
    differ must reproduce Octave; a shifted index will not."""
    d = fixture("kalman_us")
    got = kalman_filter(d["Y"], _ssm_from(d, "SSM"))
    assert np.allclose(got.filter_mu, d["filt__mu"], rtol=1e-10)
...
def test_kalman_handles_an_all_missing_period():
    rng = np.random.default_rng(12)
    n, m, t = 3, 2, 15
    ssm = StateSpace(
        D=np.zeros(n), H=rng.standard_normal((n, m)), Sigma_eps=1e-2 * np.eye(n),
        C=np.zeros(m), F=0.5 * np.eye(m), G=np.eye(m), Sigma_eta=np.eye(m),
        mu_1=np.zeros(m), Sigma_1=np.eye(m),
    )
    y = rng.standard_normal((n, t))
    y[:, 6] = np.nan
    got = kalman_filter(y, ssm)
    assert np.isnan(got.error[:, 6]).all()
...
**Model: Opus 5 for `construct_ssm`, Fable 5 for `construct_prior`.**

`construct_ssm` is intricate but fully pinned by the `construct_ssm_us` fixture — Opus territory. `construct_prior` is where a wrong hyperparameter produces a model that samples happily and is quietly mis-specified; the fixture pins the US values but gives no protection once the Australian panel changes `n` and `n_f`. That asymmetry is exactly the Fable case.
...
from nyfed.model import construct_prior, construct_ssm


@pytest.mark.fixtures
def test_construct_ssm_matches_octave_on_every_matrix(fixture):
    d = fixture("construct_ssm_us")
    got = construct_ssm(_params_from(d), _latent_from(d), _restrict_from(d))
    for name in ("D", "H", "Sigma_eps", "F", "G", "Sigma_eta", "mu_1", "Sigma_1"):
        assert np.allclose(getattr(got, name), d[f"SSM__{name}"], rtol=1e-10), name
...
def test_quarterly_rows_carry_the_mariano_murasawa_weights(fixture):
    """Rows for quarterly series load on 5 trend lags as [1,2,3,2,1]/9;
    monthly rows load on the current period only."""
    d = fixture("construct_ssm_us")
    restrict = _restrict_from(d)
    got = construct_ssm(_params_from(d), _latent_from(d), restrict)
    h0 = got.H[:, :, 0]
    isq = restrict.isquart
    iota = restrict.iota
    expected_q = np.outer(iota[isq], np.array([1, 2, 3, 2, 1]) / 9)
...
def test_covid_factor_is_masked_outside_the_pandemic_window(fixture):
    """f_active zeroes the COVID column of H outside Mar-2020..Dec-2021."""
    d = fixture("construct_ssm_us")
    restrict = _restrict_from(d)
    got = construct_ssm(_params_from(d), _latent_from(d), restrict)
    i_cov = 4
...
def test_construct_prior_matches_octave(fixture):
    ...
    got = construct_prior((31, 5, 4, 1), d["m_Lambda"])
    for name in ("m_mu", "P_mu", "m_Lambda", "P_Lambda", "m_Phi", "P_Phi",
                 "m_phi", "P_phi"):
        assert np.allclose(getattr(got, name), d[f"prior__{name}"], rtol=1e-10), name
    for name in ("nu_g", "s2_g", "nu_f", "s2_f", "nu_e", "s2_e", "a_f", "b_f",
                 "a_e", "b_e"):
        assert getattr(got, name) == pytest.approx(float(d[f"prior__{name}"]), rel=1e-12), name


def test_prior_outlier_probability_encodes_one_outlier_per_two_years():
    """pi_mean = 1 - 1/(2*12) with 20 pseudo-observations. If the Australian
    panel changes this, it must be a deliberate, documented choice."""
    prior = construct_prior((31, 5, 4, 1), np.zeros((31, 5)))
    assert prior.a_f / (prior.a_f + prior.b_f) == pytest.approx(1 - 1 / 24)
    assert prior.a_f + prior.b_f == pytest.approx(20)
    assert (prior.a_e, prior.b_e) == (prior.a_f, prior.b_f)
...
Add `_params_from`, `_latent_from` and `_restrict_from` helpers at the top of the test module that rebuild the dataclasses from the flattened fixture keys.
...
def test_update_vol_mixture_posteriors_match_octave(fixture):
    ...
    got = update_vol(d["x"], d["sigma"], float(d["gamma"]),
...
def test_density_nowcast_is_centred_on_the_point_nowcast(fixture):
    ...
    point = point_nowcast(d["Y_old"], d["Y_new"], _ssm(d, "SSM_old"),
                          _ssm(d, "SSM_new"), int(d["i_now"]) - 1, t_now)
    draws = np.array([density_nowcast(d["Y_new"], _ssm(d, "SSM_new"),
                                      int(d["i_now"]) - 1, t_now, rng)
                      for _ in range(500)])
