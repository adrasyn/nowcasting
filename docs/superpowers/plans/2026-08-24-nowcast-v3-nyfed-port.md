...
- **Two-tier test rule.** Tier 1 (deterministic functions): must match Octave fixtures to `rtol=1e-10` **with an explicit `atol`** — see the amendment below. Tier 2 (stochastic functions): test the conditional distribution's moments against analytic values, plus an end-to-end posterior-mean check within Monte Carlo error. Never assert exact equality on a draw.
- **Always pass `atol` explicitly to `np.allclose`. Amended 2026-08-25, after the sweep at `cc98e8b`.** `np.allclose` tests `|a - b| <= atol + rtol * |b|` and defaults `atol` to `1e-8`, so with `rtol=1e-10` the absolute term dominates for any value below magnitude 100 — nearly every number in this model. Every Tier 1 assertion written `np.allclose(got, want, rtol=1e-10)` was therefore an absolute-1e-8 check, not the relative check it claimed: on the `weights` array, whose smallest entry is `1.24e-3`, it permitted about 1e-5 of *relative* slop. Twenty-nine sites across Tasks 1-8 were affected.

  **Default to `atol=0.0`.** Three of the thirty-two sites cannot use it, and the reason is worth understanding before writing a new one. `prediction__gain`, `MSEs_sub__states` and `MSEs_sub__shocks` each contain entries five to seven orders of magnitude below their own array's scale, where a one-ulp difference reads as a large *relative* error: at the worst gain cell, `want = 4.952386e-07` and `got = 4.952386e-07` differ by `3.8e-16` — one ulp against an array scale of `4.235` — which is a relative deviation of `7.8e-10`. Normwise the port agrees with the oracle to between `2e-15` and `1.9e-13` on those arrays, so this is machine precision, not a discrepancy. Those three sites use `atol = 1e-12 * nanmax(|want|)`, **derived from the fixture array in code rather than hard-coded**, with the measured deviation and margin in an inline comment.

  **The claim this corrects.** Before the sweep, "Tier 1 is pinned to `rtol=1e-10`" was not true of this suite, and appeared true only because the default absolute tolerance was masking it. A pure relative tolerance is not attainable on any array containing near-zero entries; say `rtol=1e-10` with a scale-justified absolute floor instead, and state the floor's basis wherever it is not zero.
...
- **B2** Block design and normalising loadings.
  ...
  **Second landmine, found in Task 7 and confirmed by execution.** `Gibbs_update.m:156-158` reads
  `if i_f == 5, sigma(i_f,:) = 1; end` — it switches stochastic volatility **off** for the fifth
  factor, by hard-coded index, because in the US panel the fifth factor is the COVID factor. The
  Python port reproduces this literally, as it must to reproduce the US result. **An Australian
  panel with five or more factors silently loses that factor's stochastic volatility**, whichever
  factor happens to land in slot five. Nothing downstream would flag it: Task 9's end-to-end gate
  nowcasts from stored estimates and never runs the sampler, and the value does reach the published
  nowcast through `GibbsResult.sigmas`. Task 7 added
  `test_the_fifth_factors_volatility_is_pinned_at_one` so the index cannot drift unnoticed, but
  that test pins the *US* behaviour — it is not a guard for Australia. Whoever designs the AU
  blocks must decide deliberately: an explicit `restrict` field naming the no-SV factor is the
  obvious fix, and it is a change to the model's interface, not a translation choice.
...
- **Assert the estimate file is present before trusting a green suite.** Found in Plan A's final review and carried here deliberately. `Estimates_2023_09_20.mat` is 21 MB and correctly gitignored — but `tests/test_end_to_end.py` skips when it is absent, and that takes **12 of the 13 end-to-end tests with it, including `test_reproduces_the_published_nowcast`, which is the whole branch's headline claim**. The suite still reports green. Plan A's own "157 passed, zero skipped" is a property of the development machine, not of the repository: on a clean checkout the gate does not run at all. So a naive Actions job would report success without ever executing the reproduction it exists to protect. Fetch the file first (release asset, per the decision above) and **fail the job if it is missing**, rather than letting the tests skip. This is the same defect class as the fixture guard Plan A's final review caught at `tests/test_fixtures_present.py` — a missing input that degrades to green instead of red — and it is the more dangerous instance, because the test it silences is the gate.
