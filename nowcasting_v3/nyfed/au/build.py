...
def pad_to_next_quarter(panel: Panel) -> int:
    ...
    PADDING IS NOT DONE BY `build_panel`. The estimation path does not want a
    panel that runs past its data, so padding is a separate step the callers
    that want a second horizon take for themselves: the weekly tool always, the
    Plan C backtest unless `--no-pad`.
    """
...
def months_with_data(panel: Panel, t: int) -> int:
