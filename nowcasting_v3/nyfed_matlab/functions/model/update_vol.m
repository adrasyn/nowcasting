function sigma = update_vol(x, sigma, gamma, mean_prior, var_prior)
% UPDATE_VOL  Update volatilities in the stochastic volatility model:
%   x_t           = sigma_t * eps_t,
%   ln(sigma_t^2) = ln(sigma_(t-1)^2) + gamma * ups_t,
%   (eps_t, ups_t) ~ N(0, 1).
%
%   SIGMA = UPDATE_VOL(X, SIGMA, GAMMA, VAR_PRIOR) returns updated
%   volatilities SIGMA based on observables X, scale GAMMA and prior for
%   initial value VAR_PRIOR:
%     X is Tx1.
%     SIGMA is Tx1.
%     GAMMA is scalar.
...
if (nargin < 4), mean_prior = 0; end
if (nargin < 5), var_prior = 1e6; end
small = 1e-10;
gamsq = gamma^2;
